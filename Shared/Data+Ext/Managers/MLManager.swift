...
class MLManager: ObservableObject {
    ...
    func tagQuestions(note: Quiz, notes: [Quiz]) -> Quiz {
      var mutableNote = note
        ...
        do {
            ...
            let filteredtagClassification = notes.filter { data in
           ...
           // let (trainingData, testingData) = data.randomSplit(by: 0.8, seed: 5)
            ...
            print(sentimentClassifier.trainingMetrics)
               let questions = filteredtagClassification.map{$0.questions}
               var allQuestions = [Question]()
               for q in questions {
                   allQuestions.append(contentsOf: q)
               }
            ...
            for index in predictions.indices {
                if predictions[index] != "none" {
                    ...
                    mutableNote.tag = predictions[index]
